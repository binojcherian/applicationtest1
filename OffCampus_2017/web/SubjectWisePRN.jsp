            ...
            <center>
                <form id="SubjectWisePRN" name="SubjectWisePRN" action="SubjectWisePRN.jsp" method="post">
                    <input type="hidden" name="SubjectBranchId" name="SubjectBranchId" value="<%=request.getSession().getAttribute("Subject") %>"
                           <center>
                            <table  border="1" width="90%" style="border-style: groove" cellspacing="0" cellpadding="3">
                                <tr  align="center">
                                    <td height="20px" bgcolor="#d9bf79" colspan="4"> <b> <font color="#a02a28"> <%=abs.getSubjectNameFromSubjectId(request.getSession().getAttribute("Subject").toString()) %></font> <a  href="MarkEntryForPRN.jsp?AV=1&Subject=<%=request.getSession().getAttribute("Subject").toString()  %>">[ Back ]</a></b></td>
                                </tr>
                                <tr>
                    ...
                    <%
                                ...
                                try
                                {
                                    String sqlPagination = Subject.getQuery() +" where   not exists(select p.PRN ,p.StudentName from StudentSubjectMark m inner join SubjectBranchMaster b1 on b1.SubjectBranchID=m.SubjectBranchId inner join SubjectMaster s1 on s1.SubjectId=b1.SubjectId where  b1.SubjectId=? and m.StudentId=p.Studentid and m.ExamId=7 and IsValid=1 and IsMalPractice!=1  ) order by PRN "+" limit " + iPageNo + "," + iShowRows + "";//limiting no of records
...
<% if (message != null) {%>
             ...
             ;                           } catch (Exception e) {
                    ...
                    <%
                                ...
                                try
                                {
                                    String sqlPagination1 = Subject.getQuery() +" where  exists(select p.PRN ,p.StudentName from StudentSubjectMark m inner join SubjectBranchMaster b1 on b1.SubjectBranchID=m.SubjectBranchId inner join SubjectMaster s1 on s1.SubjectId=b1.SubjectId where  b1.SubjectId=? and m.StudentId=p.Studentid and m.ExamId=7 and IsValid=1 and IsMalPractice!=1  ) order by PRN "+" limit " + iPageNo + "," + iShowRows + "";
