<%-- 
    Document   : DeleteUser
    Created on : Sep 30, 2011, 4:56:09 PM
    Author     : user
--%>
...
String IsDelete="",Privilege="-1",UserName="";
boolean IsDeleted=false;
if(request.getParameter("IsDelete")!=null && request.getParameter("IsDelete").equals("1"))
    {
...
if(request.getParameter("No")!=null && request.getParameter("No").equals("No"))
    {
    response.sendRedirect("UserCreation.jsp");
    }
if(request.getParameter("submit")!=null && request.getParameter("submit").equals("Yes"))
    {
    if(User.Deleteuser(request.getParameter("UserName"), request.getParameter("Privilege")))
        {
        if(User.DeleteUserFromAssistantSOMapping(request.getParameter("UserName"), request.getParameter("Privilege")))
            {
            response.sendRedirect("UserCreation.jsp");
            }
        }
...
<div class="textblack"></div>	</td>
        <td bgcolor="#ffffff">&nbsp;
            <%-- Content --%>
            <form id="DeleteUser" name="DeleteUser" action="DeleteUser.jsp" method="post">
                <input type="hidden" name="Privilege" id="Privilege" value="<%=Privilege%>"
                <center>
                    <table  border="1" width="80%" >
                    <tr  align="center">
                        <td height="20px" bgcolor="#d9bf79" > <b> <font color="#a02a28">Disable User</font></b></td>
                    </tr>
                     
                    <tr><td colspan="2">
                 <fieldset ><legend><span style="color:#a02a28; font-size: small"><b>Disable User</b></span></legend>
                   <table>

                    <tr>
                        <td class="textblack">User Name</td>
                        <td>
                            <input type="text" id="UserName" name="UserName" readonly="true" value="<%=UserName %>" maxlength="30" style="width:215 px">
                        </td>
                    </tr>
                     <tr>
                         <%
...
{
    Des="Assistant Registrar";
}
                         %>
                        <td class="textblack">Category</td>
                        <td>
                            <input type="text" id="Category" name="Category" readonly="true" value="<%=Des %>" maxlength="30" style="width:215 px">
                        </td>
                    </tr>
                    <tr>
                        <td><font color="#a02a28">Do you want to Disable this user?</font></td>
                        <td>
