<%--
    ...
    Author     : mgu
--%>
<%@page contentType="text/html" pageEncoding="UTF-8" errorPage="ErrorPage.jsp" import="Controller.*,java.util.ArrayList,Entity.User,Entity.CourseData,model.*" %>
<%
...
model.Absentees abt=new Absentees();
UserToCourse Map=new UserToCourse(request, response);
%>
<html><head>
		<title>DEMS</title>
...
<div class="textblack"></div>	</td>
        <td bgcolor="#ffffff">&nbsp;
            <%-- Content --%>
            <form id="UserCourseMapping" name="UserCourseMapping" action="UserCourseMapping.jsp" method="post">

                <center>
                    <table  border="1" width="80%" >
                    <tr  align="center">
                        <td height="20px" bgcolor="#d9bf79" > <b> <font color="#a02a28">User-Course Mapping</font></b></td>
                    </tr>
                    <tr> <td align="center">
                            <fieldset >
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="SO" id="SO" style="width: 215px" onchange="ChangeUsers();">
                                <%
                        ...
                        <td>
                            <select name="Course" id="Course" style="width: 215px">
                                <option value="-1">.....Select.....</option>
                                <%
                                ArrayList<CourseData> CList=new ArrayList<CourseData>();
                     CList=abt.getAllBranchList();
                     Count= CList.size();i=0;
                     while(i<Count)
                        {
                        CourseData us=CList.get(i);
                                %>
                                <option  value="<%=us.BranchId %>"><%=us.BranchName %></option>
                                <% i++; }%>
                            </select>
                        </td>
                    ...
                    <%if(Map.CourseError()!=null){ %>
                    <tr>
                        <td colspan="3" align="right"><span style="color:#a02a28; font-size: small"><b><%=Map.CourseError() %></b></span> </td>
                    </tr>
                    <% } %>
                     <%if(Map.AssignCourseError()!=null && !Map.AssignCourseError().isEmpty()){ %>
                    <tr>
                        <td colspan="3" align="right"><span style="color:red; font-size: small; "><b><%=Map.AssignCourseError() %></b></span> </td>
                    </tr>
                    <% } %>
                             ...
                             <fieldset ><legend><span style="color:#a02a28; font-size: small"><b>Course-User List</b></span></legend>
                       <div id="UserList">
                       <%
                       String UserSO=null;
                    ...
                    UserMap=Map.getAssignedCourses(UserSO);
                   ...
                   <center>
                       <table>
                          <tr class="textblack"  align="center"><span style="color:#a02a28; font-size: small"><b>No Course Assigned</b></span></tr>
                       </table>
                   </center>
                   ...
                   <% }else{%>
                    ...
                    <tr>
                        ...
                        <td class="textblack"><%=us.branchDisplay %></td>
                        <td class="textblack"><%=us.UserName  %></td>
                        <td class="textblack"><a href="DeleteCoursesofUsers.jsp?Id=<%=us.AssistantSOId %>">Delete</a> </td>
                    </tr>
<%i++; }}%>
                </table>
                            </div></fieldset>
