...
public boolean UserToCourse(String Course,String SO) throws SQLException
{
    Connection con=null;
        ...
        {
            con=new DBConnection().getConnection();
            PreparedStatement st = con.prepareStatement("INSERT INTO `DEMS_db`.`UserCourseMapping`(`UserName`,`BranchId`,`StartDate`)VALUES(?,?,sysdate())");
            st.setString(1, SO);
...
public String  IsExistData(String Course,String SO) throws SQLException
    {  Connection con=null;
        try {
            con = new DBConnection().getConnection();
            PreparedStatement Query = con.prepareStatement("SELECT count(*) FROM UserCourseMapping where UserName=? and BranchId=? and EndDate is null");
            Query.setString(1, SO);
            ...
            if (Record.next())
            {
                ...
                if (i >= 1)
                {
                 return "This Course is already Mapped for the User";
                }
            }
            return null;
        } catch (SQLException ex) {
...
public String IsUserAlreadyExists(String Username) throws SQLException
{
     Connection con = null;
      try {
...
public boolean RemoveCourseFromUser(String UserCourseId) throws SQLException
{
    Connection con=null;
        ...
        {
            con=new DBConnection().getConnection();
            PreparedStatement st = con.prepareStatement("update UserCourseMapping set EndDate=sysdate() where UserCourseId=?");
            st.setString(1, UserCourseId);
            st.execute();
            return true;
...
public ArrayList<User>getAllCoursesofUser(String SO) throws SQLException
{
    ArrayList<User> UserList=new ArrayList<User>();
    Connection con=null;
        try
        {
            con=new DBConnection().getConnection();
            PreparedStatement st = con.prepareStatement("select distinct m.UserName,m.Name, a.UserCourseId,a.BranchId,d.DisplayName from MguClientLogin m,UserCourseMapping a ,BranchMaster d where a.UserName=m.UserName and m.Privilege=32 and m.EndDate is null and a.EndDate is null and a.BranchId=d.BranchId and a.UserName=? ");
            st.setString(1, SO);
            ResultSet rs=st.executeQuery();
            while(rs.next())
            {
                ...
                user.Name=rs.getString(2);
                user.AssistantSOId=rs.getString(3);
                user.BranchId=rs.getString(4);
                user.branchDisplay=rs.getString(5);
                UserList.add(user);
            }
            return UserList;
        }
...
public User getAssigedCourseRowToDelete(String CourseSOId) throws SQLException
{
     User user=new User();
    Connection con=null;
        try
        {
            con=new DBConnection().getConnection();
            PreparedStatement st = con.prepareStatement("SELECT u.UserName,u.BranchId,b.DisplayName FROM `DEMS_db`.`UserCourseMapping` u,BranchMaster b  where u.BranchId=b.BranchId and UserCourseId=?");
            st.setString(1, CourseSOId);
            ResultSet rs=st.executeQuery();
            while(rs.next())
            {
                user.UserName=rs.getString(1);
                user.BranchId=rs.getString(2);
                user.branchDisplay=rs.getString(3);
...
public int getTotalCountOfStudentsRegisteredForExam() throws SQLException
{
     Connection con=null;
        ...
        {
            con = new DBConnection().getConnection();
            PreparedStatement st=con.prepareStatement("select count(distinct StudentId) as Count from ExamRegistrationMaster where ExamId=1");
