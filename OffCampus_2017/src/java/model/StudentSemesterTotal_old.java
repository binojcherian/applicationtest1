...
public class StudentSemesterTotal_old
{
...
*/
   /*  public void SaveStudentSemesterTotal() throws SQLException
    ...
    public void SaveStudentSemesterTotal() throws SQLException
    {
        ...
        try
        {
            ...
            PreparedStatement st=con.prepareStatement("select m.StudentId,sum(m.ExternalMark+m.InternalMark+m.ModerationExt+m.ClassModeration+m.PracticalMark+m.ModerationInt) as Total,"
                    ...
                    + " b.CurrentYearSem=1 and b.Branchid=9 "
                    ...
                    + " and b1.CurrentYearSem=1 and b1.BranchId=6 and b1.IsAddedWithTotal=1 "
        ...
        {
            Logger.getLogger(StudentSemesterTotal_old.class.getName()).log(Level.SEVERE, null, ex);
        }
        finally
        {
            con.close();
     ...
     public void SaveStudentSemesterTotal_1(String BranchId,String YearSem) throws SQLException
    {
        Connection con=null;
        try
        {
            con=new DBConnection().getConnection();
            PreparedStatement st=con.prepareStatement("select m.StudentId,sum(m.ExternalMark+m.InternalMark+m.ModerationExt+m.ClassModeration+m.PracticalMark+m.ModerationInt+m.ExternalGraceMark+m.InternalGraceMark) as Total,b.AcademicYear,"
                    ...
                    + " where StudentId =m.StudentId and b2.CurrentYearSem="+YearSem+" and b2.BranchId="+BranchId+" ) as ExamId, b.CurrentYearSem,b.BranchId from StudentSubjectMark m "
                    + " inner join SubjectBranchMaster b on m.SubjectBranchId=b.SubjectBranchId and "
                    + " b.CurrentYearSem="+YearSem+" and b.Branchid="+BranchId+""
                    + " where MarkStatus=1 and IsValid=1 and b.IsAddedWithTotal=1 "
                    + " and m.StudentId not in( select distinct StudentId  from StudentSubjectMark s "
                    + " inner join SubjectBranchMaster b1  on s.SubjectBranchId=b1.SubjectBranchId "
                    + " and b1.CurrentYearSem="+YearSem+" and b1.BranchId="+BranchId+" and b1.IsAddedWithTotal=1 "
                    + " where (s.IsAbsent=1 or s.IsPassed=0 or s.IsMalPractice=1 or s.IsPracticalAbsent=1  ) and MarkStatus=1 and IsValid=1) "
                    + " group by m.StudentId");
            ...
            while(Rs.next())
            {
                st=con.prepareStatement("INSERT INTO StudentPassDetails (StudentId,ExamId,YearOrSem,TotalMark,EnteredDate,LastModifiedDate,BranchId,Syllabus) values(?,?,?,?,sysdate(),sysdate(),?,?)");
                st.setInt(1, Rs.getInt("StudentId"));
                st.setString(2, Rs.getString("ExamId"));
                st.setInt(3, Rs.getInt("CurrentYearSem"));
                st.setInt(4, Rs.getInt("Total"));
                st.setInt(5, Rs.getInt("BranchId"));
                st.setInt(6, Rs.getInt("AcademicYear"));
                st.execute();
            }
        }
        catch (SQLException ex)
    ...
    {
        StudentSemesterTotal_old SemTotal=new StudentSemesterTotal_old();
        SemTotal.SaveStudentSemesterTotal_1( "21","4");
    }
}
