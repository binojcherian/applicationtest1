    ...
    Connection con=null;
        try
        {
            ...
            PreparedStatement st = con.prepareStatement(" SELECT S.`SubjectBranchId`, S.`SubjectId`,sm.SubjectName FROM SubjectBranchMaster S"
                    ...
                    + "AcademicYear=(select max(AcademicYear) from SubjectBranchMaster where BranchId=?) And  (S.`CurrentYearSem`=3 or S.`CurrentYearSem`=4  ) ");
        ...
        {
            Logger.getLogger(mSubjectb.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        }
    finally
    {
