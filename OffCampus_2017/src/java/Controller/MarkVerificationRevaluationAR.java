...
public class MarkVerificationRevaluationAR
{
    ...
    public MarkVerificationRevaluationAR(HttpServletRequest request,HttpServletResponse response) throws SQLException
    {
        ...
        if(request.getParameter("Submit")!=null && request.getParameter("Submit").equals("Approve"))
        {
            ...
            if(request.getParameterValues("Select")!=null)
                ...
                String Stu[]=request.getParameterValues("Select");
                 ...
                 if(mark.ApproveStudentsMarkByARRevaluation(Studentmark))
                 {
                     if(mark.SaveMarkApprovalLogForARRevaluation(Studentmark, request))
                     {
                         IsVerified=true;
                     }
                 }
...
public String forRevaluationAR()
{
    Query="select distinct sp.StudentName,sp.PRN,s.SubjectName,sb.SubjectBranchId ,sm.ExternalMark,sm.InternalMark,sm.PracticalMark,sm.ModerationExt,sm.ModerationInt,sm.StudentSubjectMarkId from StudentPersonal sp,StudentSubjectMark sm,SubjectBranchMaster sb,SubjectMaster s,MarkEntryLog m " ;
         Query+="where sp.StudentId=sm.StudentId and sm.IsARVerifiedRevaluation=1 and sm.SubjectBranchId=sb.SubjectBranchId and s.SubjectId=sb.SubjectId and sm.StudentSubjectMarkId=m.StudentSubjectMarkId  and sm.IsAssistantVerified=1 and sm.IsSOVerified=1 and IsValid=1  and sm.forRevaluationSO=0  ";
         if(Search)
         {
...
{
    return mark.getPostCorrectionByAR(forRevaluationAR());
}
public int getTotalCountOfRecordsToBeVerifiedByAR() throws SQLException
{
    return mark.getTotalCountOfRecordsToBeVerifiedByRevaluation();
}
...
public boolean UpdateMarksByARRevaluation(String StudentSubjectMarkId) throws SQLException
{
