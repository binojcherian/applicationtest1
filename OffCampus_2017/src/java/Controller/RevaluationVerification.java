...
public class RevaluationVerification
{
   ...
   Double external= -1.0,internal= -1.0,externalmod=0.0,internalmod=0.0,practical=0.0;
    ...
    public RevaluationVerification(HttpServletRequest request,HttpServletResponse response) throws SQLException, IOException
    {
        ...
        if(request.getParameter("Submit")!=null && request.getParameter("Submit").equals("Approve"))
        {
            ...
            if(request.getParameterValues("Select")!=null)
                ...
                String Stu[]=request.getParameterValues("Select");
                 ...
                 if(mark.ApproveStudentsMarkBySORevaluation(Studentmark))
                 {
                     if(mark.SaveMarkApprovalLogForSORevaluation(Studentmark, request))
                     {
                         IsVerified=true;
                     }
                 }
...
" where  m.forRevaluationSo=1 and m.IsValid=1 and u.EndDate is null and  m.IsAssistantVerified=1 and m.IsSOVerified=1  ";
         //if(Search)
        // {
...
public ArrayList<StudentSubjectMark> getRevaluationBySO() throws SQLException
{
    return mark.getRevaluationBySO(forRevaluation(), request.getSession().getAttribute("UserName").toString());
}
...
public int getTotalCountOfRecordsToBeVerifiedBySORevaluation() throws SQLException
{
    return mark.getTotalCountOfRecordsToBeVerifiedBySORevaluation(request.getSession().getAttribute("UserName").toString());
}
public boolean UpdateMarksBySO(String StudentSubjectMarkId) throws SQLException
{
    ...
    public boolean UpdateMarksBySORevaluation(String StudentSubjectMarkId) throws SQLException {
