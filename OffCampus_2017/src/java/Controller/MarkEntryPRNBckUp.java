...
public class MarkEntryPRNBckUp
{
    HttpServletRequest request;
    HttpServletResponse response;
    String UserName=null;
    String ExamId="-1",BranchId="-1",Sem="-1",Id=null,SubjectBranchId="-1",SubjectId="-1",AcdemicYear="-1",PRN=null,Remarks=null,ExamError=null,BranchError=null,SubjectError=null,AcdemicYearError=null,PRNError=null,SemError=null,MarkError=null,InternalError=null,PracticalError=null,RemarksError=null;
    
    Double Mark= -1.0;Integer Internal=-1;
    Double Practical=-1.0;
    ...
    public MarkEntryPRNBckUp(HttpServletRequest request,HttpServletResponse response) throws SQLException, UnknownHostException
    {
        ...
        if(request.getParameter("Remarks")!=null && !(request.getParameter("Remarks").isEmpty()))
    {
        ...
        if(request.getParameter("Subject")!=null && request.getParameter("AV")!=null)
        {
            ...
            Sem=request.getSession().getAttribute("Sem").toString();
            FillSubjectDetails();
                ...
                if(SemError==null)
                {
                    ...
                    if(SubjectError==null)
                    {
                        ...
                        if(PRNError==null)
                        {
                           ...
                           if(MarkError==null)
                           {
                               InternalError=getInternalMarkError();
                               ...
                               {
                                   if(IsSubjectHasPractical()){
                                   PracticalError=getPracticalMarkError();
                                    if(PracticalError==null)
                               {
                              IsSaved=mark.SaveStudentMarkForSubject(absent.getStudentIdFromPRN(PRN), ExamId, SubjectBranchId,Mark,Internal,Practical,Remarks);
                              if(IsSaved)
                              {
                           ...
                           if(MarkError==null)
                           {
                               ...
                               if(RemarksError==null)
                               {
                                   if(IsSubjectHasPractical()){
                                    ...
                                    if(PracticalError==null)
                               {
                              IsSaved=mark.UpdateMarkofStudent(absent.getStudentIdFromPRN(PRN), ExamId, SubjectBranchId,Mark,Internal,Practical,Remarks,Id);
                              if(IsSaved)
                              {
...
//                                      }
    ...
    private void FillDataForEdit() throws SQLException
    {
        ...
        AcdemicYear=studentMark.AcdemicYear;
        Sem=studentMark.Sem;
        ...
        Practical=Double.parseDouble(studentMark.PracticalMark);               
...
public String getSemYear()
{
   if(Sem!=null){
    ...
    if( request.getSession().getAttribute("Sem")!=null )
    return request.getSession().getAttribute("Sem").toString();
    else
        return "-1";
}
...
public String getExam()
{
    ...
    return request.getSession().getAttribute("ExamId").toString();
    else
        return ExamId;
}
...
{
    if(BranchId!=null){return BranchId;}else
    if(request.getSession().getAttribute("BranchId")!=null)
...
public String getSubject()
{
   if(SubjectId!=null){
    ...
    if(request.getSession().getAttribute("SubjectBranchId")!=null)
    return request.getSession().getAttribute("SubjectBranchId").toString();
    else
...
public String getAcademicYear()
{
    ...
    return AcdemicYear;
     ...
     if(request.getSession().getAttribute("AcdemicYear")!=null)
        return request.getSession().getAttribute("AcdemicYear").toString();
     else
    return "-1";
}
...
{
    int Abs=absent.getTotalCountOfAbsenteeForSubject(SubjectId,ExamId,BranchId,Sem,AcdemicYear);
...
public String getPRNError() throws SQLException
{
    ...
    else if(!mark.IsStudentRegisteredThisSubject(absent.getStudentIdFromPRN(PRN), SubjectId,ExamId,BranchId,Sem,AcdemicYear))
    {
        return "Student Does not Registered to Selected Subject";
    }
    else if(mark.IsStudentMarkAlreadyEntered(absent.getStudentIdFromPRN(PRN), SubjectId,ExamId,BranchId,Sem,AcdemicYear))
    {
    ...
    else if(mark.IsStudentUnconfirmedForSubject(absent.getStudentIdFromPRN(PRN),ExamId,BranchId,SubjectId,Sem,AcdemicYear))
    {
...
{
    SubjectBranchId=mark.getStudentSubjectBranchId(ExamId,absent.getStudentIdFromPRN(PRN),SubjectId,Sem);
    if(PRN==null ||PRN.isEmpty())
    {
        return "Enter PRN";
    }
...
{
    return absent.getBranchListForMBA();
}
...
public String getSubjectNameFromSubjectId() throws SQLException
{
   return absent.getSubjectNameFromSubjectId(SubjectId);
}
...
{
    return absent.getTotalAbsenteesEnteredForSubjectWithoutFee(SubjectId,ExamId,BranchId, Sem,AcdemicYear);
}
public int getTotalMalpracteesEnteredForSubjectWithoutFee() throws SQLException
{
    return absent.getTotalMalpracteesEnteredForSubjectWithoutFee(SubjectId,ExamId,BranchId, Sem,AcdemicYear);
}
...
{
    return mark.IsStudentAbsentForSubject(absent.getStudentIdFromPRN(PRN),ExamId,BranchId,SubjectId,Sem,AcdemicYear);
}
...
{
    return mark.CalculateInternal(Mark, SubjectId,BranchId,Sem,AcdemicYear);
}
...
public boolean IsSubjectHasExternal() throws SQLException
{
    SubjectBranchId=mark.getStudentSubjectBranchId(ExamId,absent.getStudentIdFromPRN(PRN),SubjectId,Sem);
    return mark.IsSubjectHasExternal(SubjectBranchId);
}
public boolean IsSubjectHasPractical() throws SQLException
{
    SubjectBranchId=mark.getStudentSubjectBranchId(ExamId,absent.getStudentIdFromPRN(PRN),SubjectId,Sem);
...
{
    return mark.IsSubjectHasNoInternalCalculation(SubjectId,BranchId,Sem,AcdemicYear);
}
