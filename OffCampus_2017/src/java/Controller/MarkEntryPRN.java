...
public class MarkEntryPRN
{
    ...
    String ExamId="-1",BranchId="-1",Id=null,Sem="-1",SubjectBranchId="-1",PRN=null,Remarks=null,ExamError=null,AcdemicYear="-1",SubjectId="-1",AcdemicYearError=null,BranchError=null,SubjectError=null,PRNError=null,SemError=null,MarkError=null,RemarksError=null;
    ...
    if(request.getParameter("Exam")!=null)
    {
        ExamId=request.getParameter("Exam");
         request.getSession().setAttribute("Exam", ExamId);
    }
     if(request.getParameter("AcdemicYear")!=null)
    {
        AcdemicYear=request.getParameter("AcdemicYear");
         request.getSession().setAttribute("Acdemic", AcdemicYear);
    }
         ...
         if(request.getParameter("iPageNo")!=null && (!request.getParameter("iPageNo").equals("0")))
    {
        ...
        {
            ExamId=request.getSession().getAttribute("Exam").toString();
        }
         if(request.getSession().getAttribute("Sem")!=null)
        {
          ...
          if(request.getSession().getAttribute("Acdemic")!=null)
        {
            AcdemicYear=request.getSession().getAttribute("Acdemic").toString();
        }
    ...
    public ArrayList<Subject> getSubjectsForCourseAcdemicYear() throws SQLException
{
    ...
    public String getApprovedQuery()
    {
        ...
        Query+=" order by s.SubjectName,sp.PRN";
        return Query;
    }
     public String getPendingPRNList()
    {
        String Query="SELECT SQL_CALC_FOUND_ROWS distinct p.PRN ,p.StudentName FROM `DEMS_db`.`ExamRegistrationMaster`  e  inner join StudentPersonal p on e.StudentID=p.Studentid  and e.ExamId=7 inner join SubjectBranchMaster b on b.SubjectBranchID=e.SubjectBranchId  inner join SubjectMaster s on s.SubjectId=b.SubjectId  ";

        if(Search)
        {
            if(ExamId!=null && (!ExamId.equals("-1")))
             {
                 Query+=" and e.ExamId="+ExamId;
             }
              if(SubjectId!=null && (!SubjectId.equals("-1")) )
             {
